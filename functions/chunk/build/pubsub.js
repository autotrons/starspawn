...
function createTopic(topic) {
  ...
  return pub_client.createTopic({ name: formattedName }).then(responses => {
...
function deleteTopic(topic) {
  const formattedName = pub_client.topicPath(PROJECT, topic);
  return pub_client.deleteTopic({ topic: formattedName }).then(responses => {
    return success(responses);
  }).catch(err => {
    return failure(err.toString());
  });
...
function createSubscription(topic, sub) {
  ...
  return sub_client.createSubscription(request).then(responses => {
    return success(responses);
  }).catch(err => {
    return failure(err.toString());
  });
...
function deleteSubscription(topic, sub) {
  const formattedName = sub_client.subscriptionPath(PROJECT, sub);
  return sub_client.deleteSubscription({ subscription: formattedName }).then(responses => {
    return success(responses);
  }).catch(err => {
    return failure(err.toString());
  });
...
function publish(topic, messagesElement) {
  ...
  return pub_client.publish(request).then(responses => {
...
function pull(sub, maxMessages) {
  ...
  return sub_client.pull(request).then(responses => {
    return success(responses);
  }).catch(err => {
    return failure(err.toString());
  });
...
function ack(sub, ackIds) {
  ...
  return sub_client.acknowledge(request).then(res => {
    return success(res);
  }).catch(err => {
    return failure(err.toString());
  });
